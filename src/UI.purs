...
import UI.Types (UIInput, UIState, UIQuery (..))
import V (Dim, decDims, toggleDim)
...
-- | the view decision.
uiComponent :: forall a m. H.Component HH.HTML UIQuery (UIInput a) Void m
uiComponent =
  H.component
  ...
  render state =
    HH.div
        ...
        [ HH.fieldset
          ...
          ( HH.legend_ [ HH.text "Dimensions" ] :
            map dimBox (toUnfoldable (decDims state.viewDec)) )
        ]

  eval :: UIQuery ~> H.ComponentDSL (UIState a) UIQuery Void m
  eval = case _ of
    Toggle dim next -> do
      s <- H.get
      H.put (toggleViewDec dim s)
      pure next
    ViewDec reply -> do
      state <- H.get
